...
int main(int argc, char** argv) {
  ...
  //Coordonnée disques
  const size_t CIRCLE_POLYGONES = 10;
  const float r = 0.5;
  float teta = 0.;
  ...
  for (size_t i = 0; i < CIRCLE_POLYGONES; i++){
    ...
    vertices.push_back(Vertex2DColor(glm::vec2(r * glm::cos(teta), r * glm::sin(teta)), glm::vec3(0, 0, 1)));
    teta = 2 * glm::pi<float>() * (i+1) / CIRCLE_POLYGONES;
    vertices.push_back(Vertex2DColor(glm::vec2(r * glm::cos(teta), r * glm::sin(teta)), glm::vec3(0, 0, 1)));
  };

  //Envoi des données
  glBufferData(GL_ARRAY_BUFFER, 3*CIRCLE_POLYGONES * sizeof (Vertex2DColor), vertices.data(), GL_STATIC_DRAW);
  ...
  while (!done) {
    ...
    glBindVertexArray(vao);
    glDrawArrays(GL_TRIANGLES, 0, 3*CIRCLE_POLYGONES);
    glBindVertexArray(0);
