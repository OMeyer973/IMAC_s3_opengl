...
//cmake ../GLImac-Template && make -j 4 && ./TP8/TP8_exo1_directional_light
...
struct EarthProgram {
    ...
    EarthProgram(const FilePath& applicationPath):
        m_Program(loadProgram(applicationPath.dirPath() + "shaders/3D.vs.glsl",
                              applicationPath.dirPath() + "shaders/earthdirectionallight.fs.glsl")) {
...
struct MoonProgram {
    ...
    MoonProgram(const FilePath& applicationPath):
        m_Program(loadProgram(applicationPath.dirPath() + "shaders/3D.vs.glsl",
                              applicationPath.dirPath() + "shaders/moondirectionallight.fs.glsl")) {
        uMVPMatrix = glGetUniformLocation(m_Program.getGLId(), "uMVPMatrix");
        uMVMatrix = glGetUniformLocation(m_Program.getGLId(), "uMVMatrix");
        uNormalMatrix = glGetUniformLocation(m_Program.getGLId(), "uNormalMatrix");
    ...
    while(!done) {
        ...
        while(windowManager.pollEvent(e)) {
            ...
            case SDL_MOUSEMOTION:                
                if (mouseDown) {
                    trackballCamera.rotateUp(e.button.x - mouseX);
                    trackballCamera.rotateLeft(e.button.y - mouseY);
                    mouseX = e.button.x;
                    mouseY = e.button.y;
                }
                break;
        ...
        //material variables
        glUniform3fv(earthProgram.uKd, 1, glm::value_ptr(earthMat.diffuse));
        glUniform3fv(earthProgram.uKs, 1, glm::value_ptr(earthMat.glossy));
        glUniform1f(earthProgram.uShininess, earthMat.shininess);
        
        //light variables
        glUniform3fv(earthProgram.uLightIntensity, 1, glm::value_ptr(sunLight.intensity));
        glm::vec3 tmpLightDir(glm::mat3(trackballCamera.getViewMatrix())*sunLight.direction);
        glUniform3fv(earthProgram.uLightDir_vs, 1, glm::value_ptr(tmpLightDir));

        //calcul des view matrix, model matrix
        glm::mat4 globalMVMatrix = trackballCamera.getViewMatrix();
        ...
        for (size_t i=0; i<randomAxes.size(); ++i) {
            ...
            glUniform3fv(moonProgram.uKd, 1, glm::value_ptr(moonMat.diffuse));
            glUniform3fv(moonProgram.uKs, 1, glm::value_ptr(moonMat.glossy));
            glUniform1f(moonProgram.uShininess, moonMat.shininess);
            
            //light variables
            glUniform3fv(moonProgram.uLightIntensity, 1, glm::value_ptr(sunLight.intensity));
            //glm::vec3 tmpLightDir(glm::mat3(trackballCamera.getViewMatrix())*sunLight.direction);
            glUniform3fv(moonProgram.uLightDir_vs, 1, glm::value_ptr(tmpLightDir));
